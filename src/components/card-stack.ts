...
export const CardStack: React.FC<CardStackProps> = ({ items, className, onCardClick }) => {
  ...
  return (
    <div className={cn("relative w-full h-full", className)}>
      ...
      <div className="relative w-full h-full bg-gradient-to-b from-[#111] to-[#000] rounded-2xl overflow-hidden">
          ...
          <AnimatePresence mode="wait">
            {items.map((item, index) => (
              <motion.div
                key={item.id}
                className={cn(
                  "absolute inset-0 bg-gray-800/80 backdrop-blur-sm border border-gray-700 rounded-2xl p-6 cursor-pointer",
